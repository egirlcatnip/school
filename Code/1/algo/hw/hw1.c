...
// On windows malloc() is necessary to allocate the arrays due to stack size
// limit overflow. Made use of long long int, to avoid overflows with lengthy
// arrays.
...
// Function Prototypes        //
////////////////////////////////
long long int insertion_sort_with_counting(int pole[], int velikost,
                                           int poradi);
long long int merge_sort_with_counting(int pole[], int velikost, int poradi);
long long int merge_insertion_sort_with_counting(int pole[], int velikost,
                                                 int poradi,
                                                 int mezni_velikost);
...
long long int _merge_sort(int array[], int p, int r,
                          long long int comparison_count, bool ascending) {
  if (p < r) {
    int q = (p + r) / 2;

    comparison_count = _merge_sort(array, p, q, comparison_count, ascending);
    comparison_count =
        _merge_sort(array, q + 1, r, comparison_count, ascending);
...
long long int _hybrid_sort(int array[], int p, int r,
                           long long int comparison_count, int mezni_velikost,
                           bool ascending) {
  if (r - p + 1 <= mezni_velikost) {
    comparison_count +=
        insertion_sort_with_counting(&array[p], r - p + 1, ascending ? 1 : 0);
  } else if (p < r) {
    int q = (p + r) / 2;

    comparison_count =
        _hybrid_sort(array, p, q, comparison_count, mezni_velikost, ascending);
    comparison_count = _hybrid_sort(array, q + 1, r, comparison_count,
                                    mezni_velikost, ascending);
...
long long int merge_insertion_sort_with_counting(int pole[], int velikost,
  ...
  comparison_count =
      _hybrid_sort(pole, 0, velikost - 1, 0, mezni_velikost, ascending);
...
// Compare arrays
void compare_arrays(int array1[], int array2[], int array3[], int len,
                    int up_to) {
  bool are_equal = true;
...
void check_parameters(int len, int sort_order, int max, int min, int up_to,
                      int mezni_velikost) {
  bool error = false;
...
void compare_speeds(long long int insertion_comp_count,
                    long long int merge_comp_count,
                    long long int hybrid_comp_count, int len) {
  ...
  if (insertion_comp_count < merge_comp_count &&
      insertion_comp_count < hybrid_comp_count) {
  ...
  } else if (merge_comp_count < insertion_comp_count &&
             merge_comp_count < hybrid_comp_count) {
    ...
    printf("Merge sort is roughly %.3f times faster than hybrid sort.\n",
           hybrid_merge_ratio);
  } else if (hybrid_comp_count < insertion_comp_count &&
             hybrid_comp_count < merge_comp_count) {
...
int main() {
  ...
  if (!array || !insertion_sort_array || !merge_sort_array ||
      !hybrid_sort_array) {
  ...
  long long int insertion_comp_count =
      insertion_sort_with_counting(insertion_sort_array, len, sort_order);
  long long int merge_comp_count =
      merge_sort_with_counting(merge_sort_array, len, sort_order);
  long long int hybrid_comp_count = merge_insertion_sort_with_counting(
      hybrid_sort_array, len, sort_order, mezni_velikost);

  // Compare the arrays to ensure they're all sorted the same
  printf("Comparing arrays:\n");
  compare_arrays(insertion_sort_array, merge_sort_array, hybrid_sort_array, len,
                 up_to);
  ...
  compare_speeds(insertion_comp_count, merge_comp_count, hybrid_comp_count,
                 len);
